...
def add_share_of_long_trades(
    ...
    for exchange in exchange_set.all_exchanges():
        feature_name: str = get_feature_name(
            feature=Feature.SHARE_OF_LONG_TRADES, exchange=exchange, window=window, prefix=prefix
        )
        # Check if feature is allowed by feature importance file
        if feature_filter.is_allowed(feature_name=feature_name):
            ts_by_name[feature_name] = read_share_long_trades(
                bounds=bounds,
                exchange=exchange,
